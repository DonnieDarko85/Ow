<?php
/*
return [
    'db' => [
        'host' => 'localhost',
        'port' => 3306,
        'database' => 'my_mondodelleleggende',
        'username' => 'mondodelleleggende',
        'password' => '',
        'charset' => 'utf8mb4',
    ],
    'app' => [
        'name' => 'Sun-Tzu Secrets Play Portal',
        'version' => '0.9',
        'organization_name' => 'Sun-Tzu Secrets',
        'legal_note' => 'Note legali, privacy e cookie sono in preparazione. Contenuti informativi e riferimenti ufficiali verranno pubblicati a breve.',
        'contact_email' => 'placeholder contatti in aggiornamento',
        'registration_mail_from' => 'play@suntzusecrets.com',
        'data_encryption_key' => 'change-me-with-a-long-random-secret',
    ],
];
*/

return [
    'db' => [
        'host' => 'localhost',
        'port' => 3306,
        'database' => 'saoplbpk_mondodelleleggende',
        'username' => 'saoplbpk_user',
        'password' => 'Eloh1m@666_',
        'charset' => 'utf8mb4',
    ],
    'app' => [
        'name' => 'Sun-Tzu Secrets Play Portal',
        'version' => '1.0.1',
        'organization_name' => 'Sun-Tzu Secrets',
        'legal_note' => 'Note legali, privacy e cookie sono in preparazione. Contenuti informativi e riferimenti ufficiali verranno pubblicati a breve.',
        'contact_email' => 'placeholder contatti in aggiornamento',
        'registration_mail_from' => 'play@suntzusecrets.com',
        'data_encryption_key' => 'f7d2c9a4e8b1f6a0c3d5e9b7a2f4c8d1e6a9b0c2d7f5e3a8c1b6d4f9a0e2c7b5',
    ],
];

